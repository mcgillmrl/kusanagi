...
'''
Implementation of the PILCO algorithm by Deisenroth et. al.
'''
import numpy as np
import os
import sys
...
class PILCO(EpisodicLearner):
    def __init__(self, params, plant_class, policy_class, cost_func=None,
        ...
        self.use_empirical_x0 = params['use_empirical_x0']\
        if 'use_empirical_x0' in params else False
    ...
    def save(self, output_folder=None, output_filename=None, save_compiled_fns=False):
        ''' Saves the state of the learner, including the parameters of the policy
    ...
    def load(self, output_folder=None, output_filename=None, load_compiled_fns=False):
        ''' Loads the state of the learner, including the parameters of the policy and\
        the dynamics model, and the compiled rollout functions'''
        ...
        if not hasattr(self, 'dynamics_model') or self.dynamics_model is None:
            self.dynamics_model = self.dynmodel_class(filename=dynamics_filename,
                                                      **self.dynmodel_params)
        else:
    ...
    def save_compiled_fns(self, output_folder=None, output_filename=None):
        ''' Saves the compiled rollout and policy_gradient functions, along with the associated
        shared variables from the dynamics model and policy. The shared variables from the dynamics
        model adn the policy will be replaced with whatever is loaded, to ensure that the compiled
        rollout and policy_gradient functions are consistently updated, when the parameters of the
        dynamics_model and policy objects are changed. Since we won't store the latest state of
        these shared variables here, we will copy the values of the policy and dynamics_model
        parameters into the state of the shared variables. If the policy and dynamics_model
        parameters have been updated, we will need to load them before calling this function.'''
        ...
        with open(path, 'wb') as f:
            ...
            # this saves the shared variables used by the rollout and policy gradients.
            # This means that the zip file will store the state of those variables from
            # when this function is called
            t_vars = [self.dynamics_model.get_state(),
                      self.policy.get_state(),
                      self.rollout_fn,
    ...
    def load_compiled_fns(self, output_folder=None, output_filename=None):
        ''' Loads the compiled rollout and policy_gradient functions, along with the associated
        shared variables from the dynamics model and policy. The shared variables from the dynamics
        model adn the policy will be replaced with whatever is loaded, to ensure that the compiled
        rollout and policy_gradient functions are consistently updated, when the parameters of the
        dynamics_model and policy objects are changed. Since we won't store the latest state of
        ...
        with open(path, 'rb') as f:
            ...
            t_vars = t_load(f)
            # here we are loading state variables that are probably outdated, but that are tied
            # to the compiled rollout and policy_gradient functions.
            ...
            # At this point the dynamics model and policy state variables should be tied to the
            # rollout and policy_graddient function, and contain the up to date values of the
            # parameters
    ...
    def get_rollout(self, mx0, Sx0, H, gamma0, cost,
        ...
        is uncertain.'''
        utils.print_with_stamp('Computing symbolic expression graph for belief state propagation',
                               self.name)
        if dynmodel is None:
            dynmodel = self.dynamics_model
        if policy is None:
    ...
    def compile_policy_gradients(self, mx0=None, Sx0=None, H=None, gamma0=None,
                                 dynmodel=None, policy=None):
        ''' Compiles a theano function graph that computes the gradients of the expected
        accumulated a given initial state and prediction horizon. The function will return
        the value of the policy, followed by the policy gradients.'''
        mx0 = self.mx0 if mx0 is None else mx0
        Sx0 = self.Sx0 if Sx0 is None else Sx0
        H = self.H_steps if H is None else H
        gamma0 = self.gamma0 if gamma0 is None else gamma0
        ...
        # the policy gradients will have the same shape as the policy parameters (i.e. this
        # returns a list of theano tensor variables with the same dtype and ndim as the parameters
        # in p )
    ...
    def rollout(self, mx0, Sx0, H_steps, discount):
        ''' Function that returns trajectory distributions from the dynamics model. Ensures the
        compiled rollout function is initialised before we can call it'''
    ...
    def policy_gradient(self, mx0, Sx0, H_steps, discount):
        ''' Function that computes the gradients of the parameters of the current policy.
    ...
    def value(self, return_grads=False):
        '''
        Returns the value of the current policy by computing long term predictions using a
        learned dynamics model. If return_grads is True, it will also return the policy gradients
        '''
        ...
        # we will perform a rollout with a horizon that is as long as the longest run,
        # but at most self.H
        ...
        if return_grads:
            ...
            # first return argument is the value of the policy, second are the gradients wrt the
            # policy params
            ret = [pg[0]]
            ret.append(pg[1:])
            return ret
