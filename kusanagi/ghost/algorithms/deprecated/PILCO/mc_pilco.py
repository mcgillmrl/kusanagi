...
class MC_PILCO(PILCO):
    ...
    def propagate_state(self, x, u=None,*args, **kwargs):
        ...
        '''
        dynmodel = self.dynamics_model if kwargs.get('dynmodel', None) is None\
                   else kwargs.get('dynmodel', None)
        policy = self.policy if kwargs.get('policy', None) is None\
        ...
        if u is None:
            ...
            u = policy.evaluate(xa_noisy, symbolic=True,
    ...
    def rollout_single_step(self, x, gamma, gamma0, *args, **kwargs):
        ...
        #jac = theano.tensor.jacobian(x_next.flatten(), x)
        #jac = jac.reshape((x.shape[0], x.shape[1], x.shape[0], x.shape[1]))
        #jac = jac.diagonal(axis1=0, axis2=2)
    ...
    def get_rollout(self, x0, H, gamma0, n_samples, dynmodel=None, policy=None, cost=None):
        ...
        rollout_single_step = partial(self.rollout_single_step,
                                      dynmodel=dynmodel,
                                      policy=policy,
                                      cost=cost)
    ...
    def get_policy_value(self, x0=None, H=None, gamma0=None,
                         n_samples=None, dynmodel=None, policy=None,
                         cost_per_sample=False):
        '''
            Returns a symbolic expression (theano tensor variable)
            for the value of the current policy
        '''
        ...
        if cost_per_sample:
            ...
            # first axis: batch, second axis: timestep
            expected_accumulated_cost = costs[:, 1:].sum(1).mean()
            expected_accumulated_cost.name = 'J'
            return expected_accumulated_cost, updts

    def compile_rollout(self, x0=None, H=None, gamma0=None,
                        n_samples=None, dynmodel=None, policy=None,
                        cost_per_sample=False):
        '''
           Compiles a theano function graph that compute the predicted
           states and discounted costs for a given inital state and prediction
           horizon. Unlike GP based PILCO, This function returns trajectory
           samples instead of means and covariances for everty time step
        '''
        x0 = self.x0 if x0 is None else x0
        H = self.H_steps if H is None else H
        gamma0 = self.gamma0 if gamma0 is None else gamma0
        ...
        rollout_fn = theano.function([], ret,
    ...
    def compile_policy_gradients(self, x0=None, H=None, gamma0=None,
                                 n_samples=None, dynmodel=None, policy=None):
        '''
            Compiles a theano function graph that computes the gradients of
            the expected accumulated a given initial state and prediction
            horizon. The function will return the value of the policy, followed
            by the policy gradients.
        '''
        if policy is None:
            policy = self.policy

        expected_accumulated_cost, updts = self.get_policy_value(x0, H, gamma0,
                                                                 n_samples,
                                                                 dynmodel, policy)
        # get the policy parameters (as theano tensor variables)
        p = policy.get_params(symbolic=True)
        # the policy gradients will have the same shape as the policy
        # parameters (i.e. this returns a list of theano tensor variables
        # with the same dtype and ndim as the parameters in p )
        #gamma0 = self.gamma0 if gamma0 is None else gamma0
